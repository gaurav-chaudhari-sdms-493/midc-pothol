...
async def create_report_route(report: report_dto.ReportCreate, db_session: Session = Depends(get_db)):
    return await report_controller.create_report(report, db_session)
...
async def get_reports_route(
    ...
    return await report_controller.get_reports(reportedBy, status, db_session)
